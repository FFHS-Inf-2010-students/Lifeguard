...
/**
 * State for awaiting the reply from the rescuer.
 * 
...
public class AwaitingState extends AbstractAlarmState {
    ...
    private final ContactInterface contact;
     ...
     * 
     * @param contact
     *            the contact to await the reply of
     */
    ...
    protected void registerReceiver() {
        ...
        getAlarmContext().getAndroidContext().registerReceiver(smsReceiver,
                filter);
    }
    ...
    protected void registerTimeout() {
        ...
        TimerTask timeout = new TimerTask() {
            ...
            public void run() {
                cancel();
                getAlarmContext().setNext(
                        new AlarmingState(contact.getPosition()));
            }
        };
        SharedPreferences prefs = getAndroidContext().getSharedPreferences(
                Lifeguard.APPLICATION_SETTINGS, 0);
        // default timeout 10min
        long delay = Long.parseLong(prefs.getString(getAndroidContext()
                .getString(R.string.alarmRepeatDelayConfigurationKey), "600")) * 1000;
    ...
    protected void receiveSms(Context context, Intent intent) {
        ...
        if (bundle != null) {
            ...
            // @see
            // http://stackoverflow.com/questions/8814751/how-to-store-messages-in-list-received-from-broadcast-receiver
            Object[] pdus = (Object[]) bundle.get("pdus");

            for (int i = 0; i < pdus.length; ++i) {
                SmsMessage message = SmsMessage.createFromPdu((byte[]) pdus[i]);
                String senderNumber = getNormalizedPhone(message
                        .getOriginatingAddress());

                if (senderNumber.equals(getNormalizedPhone(contact.getPhone()))) {
                    Log.d(getClass().toString(), contact.getName()
                            + " has responded to alarm (" + contact.getPhone()
                            + ").");
                    helpConfirmed = true;
                    break;
                }
